...
function rng_get_order_ids_by_product_id($product_id){
    global $wpdb;
...
function rng_check_order_status($order){
...
function get_customer_ids_by_product_id($product_id){
    $order_ids = rng_get_order_ids_by_product_id($product_id);
    if(empty($order_ids)){
        return false;
    }
    $customer_ids = array();
    foreach($order_ids as $order_id){
        ...
        if(false == rng_check_order_status()){
            continue;
        }
